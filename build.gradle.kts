...
// SPEC-419 TEMP (revert before SDK PR) — build+install nurrai-android, writing a marker
// file the bridge can /fs/read, so a Cloudflare 524 (tunnel ~100s < build ~120s) never hides
// the compile result.
...
tasks.register<Exec>("emuRestart") {
    commandLine("bash", "-lc", """
        set +e
        AH="/opt/homebrew/share/android-commandlinetools"
        EMU="${'$'}AH/emulator/emulator"; ADB="${'$'}AH/platform-tools/adb"
        launchctl remove com.appdna.emu34 2>/dev/null; sleep 2
        pkill -f "avd appdna34" 2>/dev/null; sleep 3
        launchctl submit -l com.appdna.emu34 -o /tmp/emu.log -e /tmp/emu.err -- "${'$'}EMU" -avd appdna34 -no-window -no-snapshot -no-audio -gpu swiftshader_indirect -no-boot-anim
        "${'$'}ADB" start-server >/dev/null 2>&1
        st=""
        for i in ${'$'}(seq 1 28); do
          st=${'$'}("${'$'}ADB" -e shell getprop sys.boot_completed 2>/dev/null | tr -d '\r')
          [ "${'$'}st" = "1" ] && break
          sleep 5
        done
        "${'$'}ADB" devices; echo "boot=[${'$'}st]"
        echo "=== RESTART DONE ==="; exit 7
    """)
    isIgnoreExitValue = false
}

tasks.register<Exec>("navMode") {
    commandLine("bash", "-lc", """
        set +e
        AH="/opt/homebrew/share/android-commandlinetools"; ADB="${'$'}AH/platform-tools/adb"
        echo "=== current nav mode (2=gesture,0=3button) ==="
        "${'$'}ADB" -e shell settings get secure navigation_mode 2>/dev/null
        echo "=== enabling gesture nav ==="
        "${'$'}ADB" -e shell cmd overlay enable com.android.internal.systemui.navbar.gestural 2>&1 | tail -2
        sleep 2
        echo "after:"; "${'$'}ADB" -e shell settings get secure navigation_mode 2>/dev/null
        echo "=== NAVMODE DONE ==="; exit 7
    """)
    isIgnoreExitValue = false
}


tasks.register<Exec>("adbReconnect") {
    commandLine("bash", "-lc", """
        set +e
        AH="/opt/homebrew/share/android-commandlinetools"; ADB="${'$'}AH/platform-tools/adb"
        "${'$'}ADB" kill-server 2>&1 | tail -1
        "${'$'}ADB" start-server 2>&1 | tail -1
        echo "before:"; "${'$'}ADB" devices
        timeout 60 "${'$'}ADB" -e wait-for-device 2>&1
        echo "boot=$("${'$'}ADB" -e shell getprop sys.boot_completed 2>&1 | tr -d '\r\n')"
        echo "after:"; "${'$'}ADB" devices
        echo "=== ADBRECONNECT DONE ==="; exit 7
    """)
    isIgnoreExitValue = false
}

tasks.register<Exec>("bootSims") {
    commandLine("bash", "-lc", """
        set +e
        ASDK=/opt/homebrew/share/android-commandlinetools
        OUT="${'$'}HOME/nurrai-android/_bootsims.txt"
        open -a Simulator >/dev/null 2>&1
        xcrun simctl boot "iPhone 17 Pro" >/dev/null 2>&1
        if ! "${'$'}ASDK/platform-tools/adb" devices 2>/dev/null | grep -q "emulator-5554"; then
          nohup "${'$'}ASDK/emulator/emulator" -avd appdna34 -no-snapshot-load >/dev/null 2>&1 &
          disown
        fi
        sleep 3
        IOS=$(xcrun simctl list devices booted 2>&1 | grep -i "iPhone 17 Pro" | head -1 | tr -s ' ')
        AND=$("${'$'}ASDK/platform-tools/adb" devices 2>&1 | grep emulator-5554)
        echo "iOS=[${'$'}IOS] AND=[${'$'}AND]" > "${'$'}OUT"
        exit 0
    """)
